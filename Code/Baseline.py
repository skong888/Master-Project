...
#tf.enable_resource_variables() # for tflite
...
file_Name = os.path.join(model_File, fTime+'-Baseline/')
...
plt.savefig(file_Name+'acc_validation.png')
plt.close()
# Loss History
...
plt.xlabel('epoch')
plt.legend(['train', 'validation'], loc='upper left')
plt.savefig(file_Name+'loss_validation.png')
plt.close()
